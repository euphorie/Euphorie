...
def setupInitialContent(site):
    ...
    if "sectors" not in present:
        site.invokeFactory("euphorie.sectorcontainer", "sectors", title="Sectors")
        mt=getToolByName(site, "portal_membership")
        mt.setMembersFolderById("sectors")
        log.info("Added sectors folder")

    sectors=site.sectors
    missing_countries=set(COUNTRIES.keys()) - set(sectors.keys())
    for country in sorted(missing_countries):
        sectors.invokeFactory("euphorie.country", country, title=COUNTRIES[country])
        log.info("Added country %s (%s)", country, COUNTRIES[country])
    ...
    if "documents" not in present:
        site.invokeFactory("Folder", "documents", title=u"Documents")
    ...
    for (code,name) in lt.listSupportedLanguages():
        ...
        if "appendix" not in docs:
            _createObjectByType("Folder", docs, "appendix", title=u"Appendix")
