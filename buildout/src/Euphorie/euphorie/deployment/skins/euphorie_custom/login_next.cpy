...
util = getToolByName(context, "plone_utils")
...
home = membership_tool.getHomeUrl()
if home is not None:
    util.addPortalMessage(_(u'Welcome! You are now logged in.'))
    return REQUEST.RESPONSE.redirect(home)
...
if came_from is not None:
    ...
    if not getToolByName(context, "portal_url").isURLInPortal(came_from):
        came_from = ''
