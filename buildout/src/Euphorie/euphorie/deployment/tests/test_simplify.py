from euphorie.deployment.tests.functional import EuphorieDeploymentTestCase


class SimplifiedPloneTests(EuphorieDeploymentTestCase):
    ...
    def testRedirectViewStillValid(self):
        ...
        view=queryMultiAdapter((self.portal, self.portal.REQUEST), name="plone_redirector_view")
...
# euphorie.deployment zcml have no effect.
    ...
    def testSharingActionNotShown(self):
        self.loginAsPortalOwner()
        actions=self.portal.portal_actions.listFilteredActionsFor(object=self.portal)
        self.failUnless("sharing" not in [action["id"] for action in actions["object"]])
