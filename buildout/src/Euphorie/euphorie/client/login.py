...
class Reminder(grok.View):
    ...
    def _sendReminder(self):
        ...
        if not account:
            self.error=_(u"Unkown email address")
            return False
        ...
        try:
            mailhost._send(site.email_from_address, account.email, mail.as_string())
    ...
    def update(self):
        ...
        self.back_url="%s/@@login?%s" % (context.absolute_url(),
                urllib.urlencode(dict(came_form=came_from)))
...
class Register(grok.View):
    ...
    def _tryRegistration(self):
        ...
        if not loginname:
            self.errors["email"]=_(u"Please enter your email address")
        if not reply.get("password1"):
            self.errors["password"]=_(u"Please enter a password")
        elif reply.get("password1")!=reply.get("password2"):
            self.errors["password"]=_(u"Passwords do not match")
        if self.errors:
            return False
        ...
        if account:
            self.errors["email"]=_(u"An account with this email address already exists.")
            return False
