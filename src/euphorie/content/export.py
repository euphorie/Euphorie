...
class ExportSurvey(grok.View):
    ...
    def exportRisk(self, parent, risk):
        ...
        if StripMarkup(risk.legal_reference):
            etree.SubElement(node, "legal-reference").text=risk.legal_reference
        etree.SubElement(node, "show-not-applicable").text="true" if risk.show_notapplicable else "false"
        if risk.type=="risk":
    ...
    def exportSolution(self, parent, solution):
        ...
        if solution.prevention_plan:
            etree.SubElement(node, "prevention-plan").text=solution.prevention_plan
