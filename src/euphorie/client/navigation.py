...
def getTreeData(request, context, phase="identification", filter=None):
    ...
    if isinstance(context, model.Module):
        if not context.skip_children:
            ...
            me["children"]=[morph(obj) for obj in context.children(filter=filter)]
        ...
        parent=parents.pop()
        siblings=[morph(obj) for obj in parent.siblings(model.Module, filter=filter)]
    ...
    if parents:
        ...
        parent=parents.pop()
        roots=[morph(obj) for obj in parent.siblings(model.Module, filter=filter)]
