 ...
 */
function GetBrowserVersion(){return Number($.browser.version.split(".",2).join(""))}function GetBrowserEngine(){var a,b=["safari","opera","msie","mozilla"];for(a=0;a<b.length;a++){if($.browser[b[a]]){return b[a]}}}var engine=GetBrowserEngine();var engine_version=GetBrowserVersion();var iphone=(navigator.userAgent.search("iPhone")!==-1);var idcount=0;function assertId(a){a=$(a);var b=a.attr("id");if(!b){idcount+=1;b="id"+String(idcount);a.attr("id",b)}return b}function verifyDependencies(a){var d=$(a),c=d.attr("class").split(" "),g,b,e,f;for(b=0;b<c.length;b++){f=c[b].split("-");if(f.length<2||f[0]!=="dependsOn"){continue}g=$(":input[name="+f[1]+"]");if(!g.length){return false}if(g.attr("type")==="radio"||g.attr("type")==="checkbox"){e=g.filter(":checked").val()}else{e=g.val()}if((f.length===2||f[2]==="on")&&!e){return false}else{if(f[2]==="off"&&e){return false}else{if(f.length>3){if(f[2]==="equals"&&f[3]!==e){return false}else{if(f[2]==="notEquals"&&f[3]===e){return false}}}}}}return true}function getDependMasters(d){var a=$(d).attr("class").split(" "),b=$(),c,e;for(c=0;c<a.length;c++){e=a[c].split("-");if(e.length<2||e[0]!=="dependsOn"){continue}b=b.add(":input[name="+e[1]+"]")}return b}function initPlaceHolders(){if(!Modernizr.input.placeholder){$(this).find("[placeholder]").each(function(){if($(this).val()===""){$(this).val($(this).attr("placeholder"))}});$("[placeholder]").focus(function(){if($(this).val()===$(this).attr("placeholder")){$(this).val("");$(this).removeClass("placeholder")}}).blur(function(){if($(this).val()===""||$(this).val()===$(this).attr("placeholder")){$(this).val($(this).attr("placeholder"));$(this).addClass("placeholder")}});$("[placeholder]").closest("form").submit(function(){$(this).find("[placeholder]").each(function(){if($(this).val()===$(this).attr("placeholder")){$(this).val("")}})})}}function initDepends(a){$("*[class*='dependsOn-']",a).each(function(){var b=$(this);if(verifyDependencies(b)){b.show()}else{b.hide()}getDependMasters(b).bind("change.depends",function(){if(verifyDependencies(b)){b.slideDown()}else{b.slideUp()}})})}function BeautyTipShow(c){var a=$(".bt-content",c).hide(),b=$("canvas",c).hide(),e=b[0].width,d=b[0].height;$(c).show();b.css({width:e*0.5,height:d*0.5,left:e*0.25,top:d*0.25,opacity:0.1}).show().animate({width:e,height:d,left:0,top:0,opacity:1},400,"easeOutBounce",function(){a.show()})}function BeautyTipHide(b,e){$(".bt-content",b).hide();var a=$("canvas",b);if(a.length===0){return}var d=a[0].width,c=a[0].height;a.animate({width:d*0.5,height:c*0.5,left:d*0.25,top:c*0.25,opacity:0},400,"swing",e)}jQuery.fn.toolTip=function(d,a){var c=function(g){var h=$(this);var e=h.add($(g));var i;h.data("btdelay.hover",true).data("btdelay.delay",0);var f=function(){var k=h.data("btdelay.hover");if(k){h.data("btdelay.delay",0)}else{var j=h.data("btdelay.delay");if(j<5){h.data("btdelay.delay",j+1)}else{clearInterval(i);e.unbind(".btdelay");h.btOff()}}};e.bind("mouseenter.btdelay",function(){h.data("btdelay.hover",true)}).bind("mouseleave.btdelay",function(){h.data("btdelay.hover",false)});i=setInterval(f,100)};var b=(typeof d==="string")?a:d;b=jQuery.extend(b,{trigger:"none",postShow:c});return this.each(function(e){var f={over:function(){if(!$(this).data("btdelay.hover")){$(this).btOn()}},out:function(){}};$(this).bt(d,b).hoverIntent(f)})};jQuery.bt.options.positions="top, right, left, bottom";jQuery.bt.options.showTip=BeautyTipShow;jQuery.bt.options.hideTip=BeautyTipHide;jQuery.bt.options.fill="#c16800";jQuery.bt.options.cornerRadius=0;jQuery.bt.options.strokeWidth=1;jQuery.bt.options.strokeStyle="#f6921d";jQuery.bt.options.shadow=true;jQuery.bt.options.shadowOffsetX=0;jQuery.bt.options.shadowOffsetY=0;jQuery.bt.options.shadowBlur=5;jQuery.bt.options.spikeLength=10;jQuery.bt.options.spikeGirth=10;jQuery.bt.options.padding=10;jQuery.bt.options.shadowColor="rgba(0,0,0,.6)";jQuery.bt.options.shadowOverlap=false;jQuery.bt.options.noShadowOpts={strokeStyle:"white",strokeWidth:2};$(".printButton").live("click",function(){window.print();return false});$(document).ready(function(){$(".jsOnly").show();var c;$(".accordion").accordion();$("input.numeric").numeric();$("legend").each(function(){$(this).replaceWith('<p class="question">'+$(this).html()+"</p>")});if(engine==="msie"&&engine_version<70){var a=function(d){$(d).css({height:"0px",width:$(d).width()+"px","padding-top":$(d).height()+"px",overflow:"hidden",filter:"progid:DXImageTransform.Microsoft.AlphaImageLoader(src='"+$(d).attr("src")+"', sizingMethod='image')"})};$("img[src$=.png]").each(function(){a(this)})}initPlaceHolders();initDepends(document);$("ul.radioList,ul.radioRow").each(function(){var d=$(this);d.find("li input").hover(function(){$(this).parents("li").addClass("hover")},function(){$(this).parents("li").removeClass("hover")}).click(function(){d.find("li").removeClass("selected");d.find("li input:checked").parents("li").addClass("selected")});$("li input:checked",d).parents("li").addClass("selected")});function b(e){var d=$(e).parents("label");if(e.checked){$(d).addClass("selected")}else{$(d).removeClass("selected")}}$("label input[type=checkbox]").click(function(){b(this)}).each(function(){b(this)});if(!(engine==="msie"&&engine_version<70)){$("*[title][rel!=fancybox]:not(form):not('.clicktip')").toolTip({shrinkToFit:true})}if(engine==="msie"&&engine_version<80){$(".clicktip").each(function(){$(this).click(function(){return false})})}else{$(".clicktip").each(function(){var d=assertId(this);$(this).bt({trigger:"click",fill:"#8293ab",strokeStyle:"#8293ab",contentSelector:"$('#"+d+"').html()"}).click(function(){return false})})}$(".focustip").each(function(){var d=$(this).attr("class").match(/target-id-([A-Za-z0-9_\-]+)/);if(d){var e=assertId(this);$("#"+d[1]).bt({trigger:["focus","blur"],contentSelector:"$('#"+e+"').html()"})}});c=$(".autofocus:first");if(c.length){c.get(0).focus()}$("a[rel=fancybox]").fancybox({transitionIn:"elastic",transitionOut:"elastic",titlePosition:"over"})});if(engine==="msie"&&engine_version<80){$("form button[type=submit]").live("click",function(){var a=$(this).attr("name"),b=$(this).attr("class").split(" ")[0];$("<input type='hidden'/>").attr("name",a).attr("value",b).appendTo(this.form);$("button[type=submit]",this.form).attr("name","_buttonfix")})}if(iphone){var updateLayout=function(){window.scrollTo(0,1)};$(document).ready(function(){setTimeout(updateLayout,100)})};var ActionPlan={onSwitchMeasure:function(a){a.preventDefault();$("#measureTabs a").removeClass("current");$(this).addClass("current");$("#ActionPlanItemForm .tab-container").removeClass("current").hide();$(this.hash).addClass("current").show()},onDeleteMeasure:function(a){a.preventDefault();a.stopPropagation();if(!confirm(delete_confirm_text)){return false}if($("#measureTabs > a").length===1){$("#ActionPlanItemForm :input:not(select)").each(function(){$(this).removeAttr("value")});$("#ActionPlanItemForm select").each(function(){$(this).children(":first").attr("selected","selected")})}else{var b=$(this).closest("a");$(b[0].hash).remove();if(b.hasClass("current")){$("#measureTabs a:first").addClass("current");$("#ActionPlanItemForm .tab-container:first").addClass("current").show()}b.remove();ActionPlan.UpdateNumbering();ActionPlan.chevronize()}},chevronize:function(){if($("#measureTabs > a").length>5){$("#addMeasureButton").hide()}else{$("#addMeasureButton").show()}},prepClone:function(b){var a=$("#measureTabs > a").length;b.find("#planning-start-day-1").attr("id","planning-start-day-"+a);b.find("#planning-start-month-1").attr("id","planning-start-month-"+a);b.find("#planning-start-year-1").attr("id","planning-start-year-"+a);b.find("#planning-end-day-1").attr("id","planning-end-day-"+a);b.find("#planning-end-month-1").attr("id","planning-end-month-"+a);b.find("#planning-end-year-1").attr("id","planning-end-year-"+a);b.find(".ui-datepicker-trigger").remove();b.find(".enablePicker").each(function(){$(this).removeClass("hasDatepicker");$(this).datepicker({showOn:"button",buttonImage:"++resource++osha.oira.images/calendar.gif",buttonImageOnly:true,dateFormat:"yy",onSelect:function(d,c){$(this).parent().find(".day").val(c.selectedDay);$(this).parent().find(".month").val(c.selectedMonth+1)}})});return $(b)},onAddMeasure:function(b){b.preventDefault();var a=$("#measureTabs a:first").clone().insertBefore(this),c=ActionPlan.prepClone($("#ActionPlanItemForm .tab-container:first").clone()).appendTo("#ActionPlanItemForm");c.find(":input:not(select)").each(function(){$(this).removeAttr("value")});c.find("select").each(function(){$(this).children(":first").attr("selected","selected")});$("#measureTabs a").removeClass("current");a.addClass("current");$("#ActionPlanItemForm .tab-container").removeClass("current").hide();c.addClass("current").show("fast",$.proxy(function(){this.find("textarea, input").each(function(){$(this).removeClass("placeholder").placeholder()});this.placeholder()},c));ActionPlan.UpdateNumbering();ActionPlan.chevronize()},findActive:function(b){var a=$.map(b.find("dt"),function(c){return $(c).hasClass("current")});return $.inArray(true,a)},UpdateNumbering:function(){var a=$("#measureTabs > a"),c=$("#ActionPlanItemForm .tab-container"),b,d,f,e;for(b=0;b<a.length;b++){d=a.eq(b);d.attr("href",a[b].hash.replace(/[0-9]+/,b+1));e=d[0].firstChild;if(e.textContent){e.textContent=e.textContent.replace(/[0-9]+/,b+1)}else{e.data=e.data.replace(/[0-9]+/,b+1)}f=c.eq(b);f.attr("id",f.attr("id").replace(/[0-9]+/,b+1))}},MeasureHasData:function(a){var b=$.map(a.find(":input:not(select)"),function(c){return Boolean($(c).val())});return $.inArray(true,b)!==-1},toggleSolutionDropdown:function(){var b=$("#standardSolutions"),a;if(b.data("euphorie.visible")){b.data("euphorie.visible",false).hide();$("#standardSolutions li").off("click.euphorie");$("html").unbind("click.fallback");return false}b.data("euphorie.visible",true);a=$(this).offset();b.css({top:a.top,left:a.left}).show();$("#standardSolutions li").on("click.euphorie",ActionPlan.addStandardSolution);$("html").bind("click.fallback",ActionPlan.toggleSolutionDropdown);return false},addStandardSolution:function(b){b.preventDefault();var a=$("#ActionPlanItemForm .tab-container.current");if(ActionPlan.MeasureHasData(a)){if(!confirm(replace_confirm_text)){return}}a.find(":input.actionPlan").val($(this).find(".actionPlan").text());a.find(":input.preventionPlan").val($(this).find(".preventionPlan").text());a.find(":input.requirements").val($(this).find(".requirements").text())},enableDatePicker:function(){$(".enablePicker").each(function(){$(this).datepicker({showOn:"button",buttonImage:"++resource++osha.oira.images/calendar.gif",buttonImageOnly:true,dateFormat:"yy",onSelect:function(b,a){$(this).parent().find(".day").val(a.selectedDay);$(this).parent().find(".month").val(a.selectedMonth+1)}})})},init:function(){$(document).on("click","#measureTabs a",ActionPlan.onSwitchMeasure).on("click","#addMeasureButton",ActionPlan.onAddMeasure).on("click","#measureTabs .delete",ActionPlan.onDeleteMeasure).on("click",".button.solutions",ActionPlan.toggleSolutionDropdown);$("#ActionPlanItemForm .tab-container:not(:first)").hide();$("#measureTabs a:first").addClass("current");$("#ActionPlanItemForm .tab-container:first").addClass("current");ActionPlan.enableDatePicker()}};