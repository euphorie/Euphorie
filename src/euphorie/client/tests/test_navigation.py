...
class FindNextQuestionTests(DatabaseTests):
    ...
    def testSkipChildren(self):
        (session, survey)=createSurvey()
        mod1=model.Module(title=u"Module 1", module_id="1", zodb_path="1", skip_children=True)
        survey.addChild(mod1)
...
class FindPreviousQuestionTests(DatabaseTests):
    ...
    def testQuestionAtSameModule(self):
        ...
        survey.addChild(sister)
        self.failUnless(navigation.FindPreviousQuestion(sister, survey) is child)
...
class EvaluationNavigationTests(DatabaseTests):
    ...
    def filter(self):
        ...
        return EvaluationView.question_filter
    ...
    def testSkipModuleIfNoRisksPresent(self):
        ...
        mod11=model.Module(title=u"Module 1.1", module_id="11", zodb_path="1/1", skip_children=False)
        mod1.addChild(mod11)
        q111=model.Risk(title=u"Risk 1.1.1", risk_id="1", zodb_path="1/1/1", identification="yes")
        mod11.addChild(q111)
        self.assertEqual(navigation.FindNextQuestion(mod1, survey, self.filter()), None)
    ...
    def testSkipModuleWithPresentTop5Risk(self):
        ...
        mod11=model.Module(title=u"Module 1.1", module_id="11", zodb_path="1/1", skip_children=False)
        mod1.addChild(mod11)
        q111=model.Risk(title=u"Risk 1.1.1", risk_id="1", zodb_path="1/1/1", risk_type="top5", identification="no")
        mod11.addChild(q111)
        self.assertEqual(navigation.FindNextQuestion(mod1, survey, self.filter()), None)

    def testSkipModuleWithPresentPolicyRisk(self):
        (session, survey)=createSurvey()
        mod1=model.Module(title=u"Module 1", module_id="1", zodb_path="1", skip_children=False)
        survey.addChild(mod1)
        mod11=model.Module(title=u"Module 1.1", module_id="11", zodb_path="1/1", skip_children=False)
        mod1.addChild(mod11)
        q111=model.Risk(title=u"Risk 1.1.1", risk_id="1", zodb_path="1/1/1", risk_type="policy", identification="no")
        mod11.addChild(q111)
        self.assertEqual(navigation.FindNextQuestion(mod1, survey, self.filter()), None)

    def testSkipRisksIfNotPresent(self):
        (session, survey)=createSurvey()
        mod1=model.Module(title=u"Module 1", module_id="1", zodb_path="1", skip_children=False)
        survey.addChild(mod1)
        q11=model.Risk(title=u"Risk 1.1", risk_id="1", zodb_path="1/1", identification="yes")
        mod1.addChild(q11)
        self.assertEqual(navigation.FindNextQuestion(mod1, survey, self.filter()), None)

    def testShowRisksIfPresent(self):
        (session, survey)=createSurvey()
        mod1=model.Module(title=u"Module 1", module_id="1", zodb_path="1", skip_children=False)
        survey.addChild(mod1)
        q11=model.Risk(title=u"Risk 1.1", risk_id="1", zodb_path="1/1", identification="no")
        mod1.addChild(q11)
        self.failUnless(navigation.FindNextQuestion(mod1, survey, self.filter()) is q11)

    def testSkipRisksIfPresentTop5(self):
        (session, survey)=createSurvey()
        mod1=model.Module(title=u"Module 1", module_id="1", zodb_path="1", skip_children=False)
        survey.addChild(mod1)
        q11=model.Risk(title=u"Risk 1.1", risk_id="1", zodb_path="1/1", risk_type="top5", identification="no")
        mod1.addChild(q11)
        self.assertEqual(navigation.FindNextQuestion(mod1, survey, self.filter()), None)

    def testSkipRisksIfPresentPolicyType(self):
        (session, survey)=createSurvey()
        mod1=model.Module(title=u"Module 1", module_id="1", zodb_path="1", skip_children=False)
        survey.addChild(mod1)
        q11=model.Risk(title=u"Risk 1.1", risk_id="1", zodb_path="1/1", risk_type="policy", identification="no")
        mod1.addChild(q11)
        self.assertEqual(navigation.FindNextQuestion(mod1, survey, self.filter()), None)
...
class ActionPlanNavigationTests(DatabaseTests):
    ...
    def testShowTop5RiskEvenIfNotPresent(self):
        ...
        mod1=model.Module(title=u"Module 1", module_id="1", zodb_path="1", skip_children=False)
        survey.addChild(mod1)
        q11=model.Risk(title=u"Risk 1.1", risk_id="1", zodb_path="1/1", identification="yes", risk_type="top5")
        mod1.addChild(q11)
        self.failUnless(navigation.FindNextQuestion(mod1, survey, self.filter()) is q11)
