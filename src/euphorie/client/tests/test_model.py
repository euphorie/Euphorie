...
class SurveySessionTests(DatabaseTests):
    def testNoChildren(self):
        ...
        root=survey.addChild(model.Module(title=u"Root", module_id="1", zodb_path="1"))
        ses.add(root)
        ses.flush()
    ...
    def testReset_NoChildren(self):
        (ses, survey)=createSurvey()
        survey.reset()
        children=ses.query(model.SurveyTreeItem.id).filter(model.SurveyTreeItem.session==survey)
...
class RiskPresentFilterTests(DatabaseTests):
    def createData(self):
        ...
        self.mod1=model.Module(title=u"Module 1", module_id="1", zodb_path="1", skip_children=False)
        self.survey.addChild(self.mod1)
        self.q1=model.Risk(title=u"Risk 1", risk_id="1", zodb_path="1/1", type="risk", identification="no")
        self.mod1.addChild(self.q1)

    def query(self):
        return self.session.query(model.SurveyTreeItem).filter(model.RISK_PRESENT_FILTER)
...
class RiskPresentNoTop5FilterTests(DatabaseTests):
    ...
    def query(self):
        return self.session.query(model.SurveyTreeItem).filter(model.RISK_PRESENT_NO_TOP5_NO_POLICY_FILTER)
...
class ModuleWithRiskFilterTests(DatabaseTests):
    ...
    def query(self):
        return self.session.query(model.SurveyTreeItem).filter(model.MODULE_WITH_RISK_FILTER)
