...
class API(object):
    ...
    def _request(self, func, url_path, **kw):
        ...
        if response.status_code != 200:
            raise RuntimeError("Server returned HTTP error %d" % response.status_code)
...
def main():
    parser = argparse.ArgumentParser(description="Euphorie client API test tool")
    parser.add_argument(
        "-s",
        "--server",
    ...
    parser.add_argument(
        "-S", "--survey", default="nl/stigas/bos-en-natuur", help="Survey path to use"
    )
    parser.add_argument(
        "-m",
        "--menu",
    ...
    user_info = api.post(
        "/users/authenticate", login=options.login, password=options.password
    )
    ...
    if user_info.get("sessions"):
        ...
        for session in user_info["sessions"]:
            print(
                " - %s, last modified on %s" % (session["title"], session["modified"])
            )
            if session["survey"] == options.survey:
                print("  -> Removing this old demo session")
                api.delete("/users/%d/sessions/%d" % (user_info["id"], session["id"]))

    # Start a new session
    print("Creating a new survey session.")
    info = api.post(
        "/users/%d/sessions" % user_info["id"],
        survey=options.survey,
        title="API demonstratie",
    )
    if info["type"] == "error":
        print("Error: %s" % info["message"])
        return 1

    company_url = "/users/%d/sessions/%d/company" % (user_info["id"], info["id"])
    ...
    if company["type"] == "dutch-company":
        ...
        company = api.put(
            company_url,
            **{
                "visit-address": {"address": "Dorpstraat 2", "city": "Ons Dorp"},
